...
* </i></b> font, and even ??? will have to be defined within the family to have meaning
...
function setCurrentFont(){
  if (strlen($this->currentBaseFont)==0){
    ...
    $this->selectFont('./lib/pdf/fonts/Helvetica.afm');
  }
...
function clipRectangle($x1,$y1,$width,$height){
  $this->objects[$this->currentContents]['c'].="\n".sprintf('%.3f',$x1).' '.sprintf('%.3f',$y1).' '.sprintf('%.3f',$width).' '.sprintf('%.3f',$height).' re W n';
}
...
function getFontHeight($size){
  if (!$this->numFonts){
    $this->selectFont('./lib/pdf/fonts/Helvetica');
  }
...
function addText($x,$y,$size,$text,$angle=0,$wordSpaceAdjust=0){
  if (!$this->numFonts){$this->selectFont('./lib/pdf/fonts/Helvetica');}
