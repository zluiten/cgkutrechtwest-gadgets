...
<xsl:stylesheet xmlns:xsl='http://www.w3.org/1999/XSL/Transform' version='1.0'> 
    ...
    <xsl:template match='/'>
        <html>
            ...
            projectCategories = ['Urban Traffic', 'Interurban Traffic', 'Parking', 'Public / Freight Transport', 'Tunnel', 'Infomobility', 'Service / Maintenance'];
            projectPositions = ['Project Manager','UTC Project Specialist','IUTC Project Specialist','PT Project Specialist','Parking Project Specialist','Technical Engineer','Civil Engineer','Traffic Signal Specialist','UTC System Specialist'];
            relationshipsDD = ]]><xsl:value-of select='/contact/relationships/ddJSON'/><![CDATA[;
            
            function csvList2Checkbox(fieldId,checkboxIdBase,csvList,checkBoxes)
            {
                a = csvList.split(',');
                ...
                for(j=0;j<checkBoxes.length;j++)
                {
                    ...
                    checked += ' id="'+checkboxIdBase+j+'" onClick="calcCategoryResult(\''+checkboxIdBase+'\')"';
                    out = out + '<INPUT TYPE="CHECKBOX" '+checked+' NAME="' + checkBoxes[j] + '"/>' + checkBoxes[j] + '<BR/>';
                }   
                e.innerHTML = out;
            }
            
            function json2DD(targetId,eltName,selected,json)
            {
                e = document.getElementById(targetId);
                
                sel = document.createElement('select');
                sel.setAttribute('name',eltName);
            ...
            function calcCategoryResult(checkboxIdBase)
            {
                i=0;
                checkednames='';
                while(e = document.getElementById(checkboxIdBase+i))
                {
                    if(e.checked == true)
                    {
                ...
                e = document.getElementById(checkboxIdBase+'Result');
...
<!-- OTHER -->
                    <tr>
                        ...
                        <td><span class='field-label'>SWARCO Project Category</span>
                            <div class='content'>
                                <div class='content'>
                                    <input type="hidden" name='other[8][label]' value="Project Category"/>
                                    <input type="hidden" name='other[8][visibility]' value="visible"/>
                                    <input id='cxResult' type="hidden" name='other[8][value]' value="">
                                        <xsl:attribute name="value">
                                    ...
                                    <script type="text/javascript"> 
                                        csvList2Checkbox('calculation2','cx',"<xsl:value-of select='/contact/other-list/other[label="Project Category"]/value'/>",projectCategories);
                                    </script>
                                    </div>
                                </div>                                
                            </div>
                    ...
                    <tr>
                        <td><span class='field-label'>Date of award</span>
                            <div class='content'>
                                ...
                                <input type="hidden" name='date[0][type]' value="once"/>
                    ...
                    <tr>
                        <td colspan="3">
                            <input type="hidden" name='groups[Project References]' value="1"/>
                ...
                <table class='table'>
                    ...
                    <table class='table'>
                        ...
                        <xsl:for-each select='/contact/relationships/relationship'>
                            <xsl:call-template name="editRelationship">
                                <xsl:with-param name="index"><xsl:value-of select='position()'/></xsl:with-param>
                            </xsl:call-template>
                        </xsl:for-each>
                        <xsl:call-template name="editRelationship"><xsl:with-param name="index">100</xsl:with-param></xsl:call-template>
                        <xsl:call-template name="editRelationship"><xsl:with-param name="index">101</xsl:with-param></xsl:call-template>
                        <xsl:call-template name="editRelationship"><xsl:with-param name="index">102</xsl:with-param></xsl:call-template>
                    </table>
                </form>
            </body>
        </html>
    ...
    <xsl:template name="editRelationship" match="relationship">
            <xsl:param name="index"/>
           
                <tr>
                    <td class=''>
                        <div>
                            <xsl:attribute name="id">relEditID<xsl:value-of select="$index" /></xsl:attribute>
                            <script type="text/javascript"> 
                                json2DD("relEditID<xsl:value-of select="$index" />","Relationship[<xsl:value-of select="$index" />][relatedToId]","<xsl:value-of select='relatedToId'/>",relationshipsDD);
                            </script>
                        </div>
                    </td>
                    <td class=''>
                        <input type="hidden">
                            <xsl:attribute name="id">re<xsl:value-of select="$index" />_Result</xsl:attribute>
                            <xsl:attribute name="value"><xsl:value-of select='description'/></xsl:attribute>
                            <xsl:attribute name="name">Relationship[<xsl:value-of select="$index" />][relationship]</xsl:attribute>
                        </input>
                        <div>
                            <xsl:attribute name="id">relEdit<xsl:value-of select="$index" /></xsl:attribute>
                            <script type="text/javascript"> 
                                csvList2Checkbox("relEdit<xsl:value-of select="$index" />","re<xsl:value-of select="$index" />_","<xsl:value-of select='description'/>",projectCategories);
                            </script>
                        </div>
                    </td>
                </tr>
